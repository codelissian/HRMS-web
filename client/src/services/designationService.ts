import { httpClientWithMethods } from '@/services/httpClient';
...
class DesignationService {
  async getDesignations(filters: GetDesignationsRequest): Promise<ApiResponse<Designation[]>> {
    const response = await httpClientWithMethods.post<ApiResponse<Designation[]>>(
  ...
  async deleteDesignation(designationId: string): Promise<ApiResponse<any>> {
    const response = await httpClientWithMethods.patch<ApiResponse<any>>(
